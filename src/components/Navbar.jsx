...
function linkClass({ isActive }) {
  return `${navBase} ${
    isActive ? "text-ocean bg-ocean/10" : "text-slate-700 hover:text-ocean"
  }`;
}
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-sand/90 backdrop-blur border-b border-black/5">
      <div className="container-content flex items-center justify-between h-16">
        <Link to="/" className="flex items-center gap-2.5" onClick={() => setOpen(false)}>
          ...
          <span className="leading-tight">
            <span className="block font-display text-base font-semibold text-ink">Gyro Governance</span>
            <span className="block text-[11px] uppercase tracking-wide text-ocean">Ethical Travel</span>
          </span>
        </Link>
        ...
        <button
          className="md:hidden inline-flex items-center justify-center h-10 w-10 rounded-md text-ink hover:bg-black/5"
